...
local SolarisLib = {
    Themes = {
        Default = {
			...
			SelectedItem = Color3.fromRGB(45,45,45),
    ...
    function TabHolder:Tab(text)
		...
		if getgenv().ConfigTable[flag] == nil then
			getgenv().ConfigTable[flag] = start
                ...
                function Slider:Set(val, dontActivate)
			...
			function ItemHold:Dropdown(text,list,def,flag,callback)
					...
					local function AddOptions(opts, dontAddExisting)
						if dontAddExisting then
							for i, v in pairs(DropMain.Holder:GetChildren()) do
								if not v:FindFirstChild("ItemText") then continue; end

								local index = table.find(opts, v.ItemText.Text)
								if index then table.remove(opts, index) end
							end
						end
					...
					function Dropdown:Refresh(opts,replace)
						if replace then
							for _,v in pairs(DropMain.Holder:GetChildren()) do
								if v:IsA"TextButton" and not table.find(opts, v.ItemText.Text) then
									v:Destroy()
									DropMain.Holder.Size = Dropdown.Toggled and UDim2.new(1,0,0,6+DropMain.Holder.Layout.AbsoluteContentSize.Y) or UDim2.new(1,0,0,0)
            ...
            function ItemHold:MultiDropdown(text,list,def,flag,callback)
                ...
                local function AddOptions(opts, dontAddExisting)
                    ...
                    for _,option in pairs(opts) do
                        ...
                        spawn(function()
                            while task.wait() do
                               Option.BackgroundColor3 = Option:GetAttribute("Selected") and SolarisLib.Themes[SolarisLib.Settings.Theme].SelectedItem or SolarisLib.Themes[SolarisLib.Settings.Theme].DropdownItem
                               DropMain.Btn.Title.TextColor3 = SolarisLib.Themes[SolarisLib.Settings.Theme].TextColor
                            end
                        end)
                    end   
                ...
                function Dropdown:Set(val)
                    ...
                    DropMain.Btn.Title.Text = text .. " - " .. table.concat(Dropdown.Value, ", ")
					for _,v in pairs(DropMain.Holder:GetChildren()) do
                        if v:IsA"TextButton" and table.find(Dropdown.Value, v.ItemText.Text) then
			...
			getgenv().ConfigTable[flag] = preset
                ...
                UserInputService.InputBegan:Connect(function(Input)
                    ...
                    if (Input.KeyCode.Name == Bind.Value or Input.UserInputType.Name == Bind.Value) and not Bind.Binding then
						if isBinary then
							BindFrame:SetAttribute("Enabled", not BindFrame:GetAttribute("Enabled"))
						end
                ...
                UserInputService.InputEnded:Connect(function(Input)
                    if Input.KeyCode.Name == Bind.Value or Input.UserInputType.Name == Bind.Value then
                        if holdmode and Holding then
							BindFrame:SetAttribute("Enabled", false)
                            Holding = false
                            callback(Holding)
                        end
                    end
                ...
                spawn(function()
                    while task.wait() do
                       BindFrame.BackgroundColor3 = BindFrame:GetAttribute("Enabled") and SolarisLib.Themes[SolarisLib.Settings.Theme].SelectedItem or SolarisLib.Themes[SolarisLib.Settings.Theme].Bind
